...
void draw() {
  ...
  for (int i = 0; i < max; i++) {
    ...
    if ((blue(webcamScaled.pixels[i]) > map(oscillator, 1, 500, 0, 255) && (green(webcamScaled.pixels[i])- 100) < map(oscillator, 1, 500, 0, 255))) {

      for(int j = 1; j < (oscillator/random(0,10)); j++) {
        //pixels[constrain(i - (800 * j), 0, max)] = blue(pixels[i]) > oscillator3 ? inverted : pixels[i];
        pixels[constrain(i - (800 * j), 0, max)] = chooseColor(webcamScaled.pixels[1], oscillator3, inverted, pinkGreen, photo.pixels[i]);
        //pixels[constrain(i % (800 * j), 0, max)] = green(pixels[i]) > oscillator2 ? inverted : pixels[i];
        pixels[constrain(i % (800 * j), 0, max)] = chooseColor(webcamScaled.pixels[1], oscillator2, color(red(photo.pixels[i]), blue(inverted), green(pinkGreen)), inverted, pinkGreen);
        //pixels[constrain(i - (800 % j), 0, max)] = red(pixels[i]) > oscillator3 ? pinkGreen : pixels[i];
        pixels[constrain(i - (800 % j), 0, max)] = chooseColor(webcamScaled.pixels[1], oscillator3, inverted, pinkGreen, color(red(photo.pixels[i]), blue(inverted), green(pinkGreen)));
      }
    }
  }
  println(millis());
